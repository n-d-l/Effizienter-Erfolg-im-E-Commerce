...
function slack_connector_woocommerce_new_order_product_sale( $channel, $order, $items ) {
	...
	$search  = array( '[order-number]', '[order-link]', '[company]', '[first-name]', '[last-name]' );
	$replace = array( '#' . $order->get_order_number(), '__MP_SC_ORDER_LINK__MP_SC_', $order->get_billing_company(), $order->get_billing_first_name(), $order->get_billing_last_name() );
	...
	$args[ 'text' ] = str_replace( '__MP_SC_ORDER_LINK__MP_SC_', slack_connector_get_slack_url( esc_url( admin_url( 'post.php?post=' . $order->get_id() . '&action=edit' ) ), __( 'View Order', 'slack-connector' ) ), $args[ 'text' ] );
	...
	$possible_fields = array ( 
		...
		'order'			=> array(
								...
								'value'	=> slack_connector_get_slack_url( esc_url( admin_url( 'post.php?post=' . $order->get_id() . '&action=edit' ) ), '#' . $order->get_order_number() ),
								'short'	=> true
							),

		'customer'		=> array(
								'title'	=> slack_connector_prepare_string_for_slack( __( 'Customer', 'slack-connector' ) ),
								'value'	=> slack_connector_prepare_string_for_slack( $order->get_billing_first_name() . ' ' . $order->get_billing_last_name() ),
								'short'	=> true
							),
