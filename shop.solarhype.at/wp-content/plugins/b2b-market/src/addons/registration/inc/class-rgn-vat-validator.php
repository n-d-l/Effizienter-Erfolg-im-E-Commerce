...
class RGN_VAT_Validator {
	...
	public function __construct( $input, $billing_country = '' ) {
		...
		$start_error_message = '<strong>' .  get_option( 'vat_options_label', __( 'VAT Number', 'b2b-market' ) ) . ':</strong> ';
		
		$this->error_msgs = array(
			'200' => $start_error_message . __( 'Successfully validated. (Whew!)', 'b2b-market' ),
			'400' => $start_error_message . __( 'API server rejects input. Have you entered a valid country code? A valid country code consists of 2 capitals, like DE for Germany. Check allowed country codes <a href="http://ec.europa.eu/taxation_customs/vies/help.html" target="_blank">here</a>.', 'b2b-market' ),
			'404' => $start_error_message . __( 'Could not establish connection to API server. Falling back to static validation. (<a href="http://ec.europa.eu/taxation_customs/vies/help.html" target="_blank">Why?</a>)', 'b2b-market' ),
			'500' => $start_error_message . __( 'Validation aborted: No input found. Input might have been rejected, or the number you have entered simply cannot be found.', 'b2b-market' ),
			'501' => $start_error_message . __( 'Invalid input: Value passed does not seem to be a string or an array. (Aka something’s wrong here technically.)', 'b2b-market' ),
			'502' => $start_error_message .  __( 'Invalid input: Array does not seem to consist of 2 strings. (Aka something’s wrong here technically.)', 'b2b-market' ),
			'550' => $start_error_message . str_replace( 'RegEx: ', '', __( 'RegEx: Country Code not recognized. Have you entered a valid country code? A valid country code consists of 2 capitals, like DE for Germany. Check allowed country codes <a href="http://ec.europa.eu/taxation_customs/vies/help.html" target="_blank">here</a>', 'b2b-market' ) ),
		);
			...
			if ( ( $this->input != NULL ) && ( ! empty( $this->input[ 0 ] ) ) && ( ! empty( $this->input[ 1 ] ) ) ) {
				...
				if ( $this->hasClient ) {
					...
					if ( ( $today_api_calls < 4 ) && ( $today_total_api_calls < 25000 ) ) {
						...
						$error_log = apply_filters( 'b2b_market_vat_id_verify_error_log', false );
						
						if ( $error_log ) {
							
							if ( $today_api_calls >= 4) {
								error_log( 'B2B MARKET VAT VALIDATOR ERROR: "A Vat ID has already been validated 4 times today as invalid" CHECK REGEX INSTEAD' );
							}

							if ( $today_api_calls >= 25000) {
								error_log( 'B2B MARKET VAT VALIDATOR ERROR: "More than 25.000 API Calls today" CHECK REGEX INSTEAD' );
							}
	...
	private function check_API() {
		...
		try {
			...
			$error_log = apply_filters( 'b2b_market_vat_id_verify_error_log', false );
			if ( $error_log ) {
				error_log( 'B2B MARKET VAT VALIDATOR ERROR: "' . $e->getMessage() . '" CHECK REGEX INSTEAD' );
			}
