...
class MarketPress_Auto_Update_B2B {
	...
	public function setup( $plugindata ) {
		...
		$this->page = 'b2b-market';
		$this->menu_hook = 'woocommerce_bm_ui_left_menu_items';
	...
	function woocommerce_tested_up_to( $matches ) {
		if ( isset( $this->plugin_data->plugin_base_name ) && ( ! is_null( $this->plugin_data->plugin_base_name ) ) && isset( $matches[ $this->plugin_data->plugin_base_name ] ) ) {
			if ( isset( $matches[ $this->plugin_data->plugin_base_name ][ 'WC tested up to' ] ) ) {
				...
				if ( $wc_tested_up_to && $current_api_version && $current_plugin_version ) {
					...
					if ( version_compare( $current_plugin_version, $current_api_version, '>=' ) ) {
						...
						if ( version_compare( $matches[ $this->plugin_data->plugin_base_name ][ 'WC tested up to' ], $wc_tested_up_to, '<' ) ) {
							$matches[ $this->plugin_data->plugin_base_name ][ 'WC tested up to' ] = $wc_tested_up_to;
						}
					}
				}
			}
		...
		if ( ! empty( $error_message ) ) {
			...
			if ( ! empty( $access_expires_date ) ) {
				...
				if ( $days >= 0 && $days <= 30 ) {
					...
					if ( $days > 1 ) {
						$message = sprintf( __( 'Your %s licence is going to expire in %s days.', 'marketpress-autoupdater' ), '<strong>' . $this->plugin_data->plugin_name . '</strong>', $days );
					} else if ( $days == 1 ) {
						$message = sprintf( __( 'Your %s licence is going to expire in one day.', 'marketpress-autoupdater' ), '<strong>' . $this->plugin_data->plugin_name . '</strong>', $days );
					} else if ( $days == 0 ) {
						$message = sprintf( __( 'Your %s licence is going to expire today.', 'marketpress-autoupdater' ), '<strong>' . $this->plugin_data->plugin_name . '</strong>', $days );
					}
	...
	public function add_menu( $menu ) {
		...
		if ( empty( $key_value ) ) {
			...
			if ( ! $is_staging ) {
				$menu = array();
				add_filter( 'bm_admin_show_cache_button', '__return_false' );
			}
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						...
						<input type="text" style="width: 400px;" name="license-key" id="license-key" value="<?php echo substr_replace( $key_value, $stars, 0, $star_length ); ?>" <?php echo empty( $key_value ) ? '' : 'disabled="disabled"'; ?> />
	...
	public function get_key() {
		...
		if ( $this->key == '' && get_site_option( 'marketpress_license' ) != '' )
			$this->key = get_site_option( 'marketpress_license', array() );
