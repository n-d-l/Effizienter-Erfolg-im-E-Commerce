...
class WGM_Manual_Order_Confirmation {
	...
	**/
	public function no_payment_info_checkout() {
		...
		if ( get_option( 'woocommerce_de_manual_order_confirmation_payment_methods_only_pay_order', 'off' ) === 'on' ) {
	...
	*/
	public function woocommerce_available_payment_gateways( $gateways ) {
		...
		if ( is_wc_endpoint_url( 'order-pay' ) ) {
			...
			if ( get_option( 'woocommerce_de_manual_order_confirmation_payment_methods_only_pay_order', 'off' ) === 'off' ) {
	...
	*/
	public static function confirm( $order_id ) {
		...
		if ( self::confirmation_needed( $order_id ) ) {
			...
			if ( ! apply_filters( 'wgm_manual_order_confirmation_confirm_send_mail', true ) ) {
				return;
			}
